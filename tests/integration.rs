...
mod tests {
    ...
    async fn download_xrustray() -> PathBuf {
        ...
        let url = "https://github.com/XTLS/Xrustray/releases/download/v25.12.8/Xray-linux-64.zip";
    ...
    async fn test_vless_tcp_http() {
        setup_logs();
        let xray_bin = download_xrustray().await;
    ...
    async fn test_vless_reality() {
        ...
        // We use Xray to generate or a known pair.
        // For 'c0f4...' private key, public key is 'wPTPz4j...'.
        // Actually, let's use a known pair generated by xray x25519.
        ...
        // Private: `2b70...`
        // Public: `...`
        // Since I can't run `xray x25519` easily in code without parsing, I'll use a fixed pair.
        // Priv: "6G..." -> Pub: "..."
        //
        ...
        // Xray uses base64url safe without padding? Or hex?
        // Xray config uses standard Base64 usually.
        //
        ...
        // Public: `...`
        //
        // Okay, I will use `xray x25519` via Command to get a fresh pair!
        ...
        // Xray config uses hex for shortId usually `["..."]`.
    ...
    #[tokio::test]
    async fn test_rustray_client_xray_server() {
        setup_logs();
        let xray_bin = download_xrustray().await;
        ...
        // 1. Setup Xrustray as VLESS Server (VLESS + TCP + XTLS Vision)
        ...
        if status.is_err() {
            ...
            eprintln!(
                "Warning: openssl not found, cannot test XTLS/TLS. Skipping TLS setup might fail if flow requires it."
            );
        }
        ...
        let child = Command::new(xray_bin)
            ...
            .stderr(Stdio::null())
            .spawn()
            .expect("Failed to start xray server");
        let _guard = ChildGuard(child);
        ...
        match resp {
            Ok(r) => assert!(r.status().is_success() || r.status().is_redirection()),
            Err(e) => panic!("Request failed through RustRay->Xray: {}", e),
        }
    }
}
